...
public class ScapegoatTree<Key extends Comparable<Key>, Value> {
    ...
    */
	...
	public List<Node<Key,Value>> preOrder() {
        List<Node<Key,Value>> sik = new ArrayList<>();
        preOrder(root, sik);
        return sik;
    }

    private void preOrder(Node<Key,Value> node, List<Node<Key,Value>> sik) {
        if(node != null) {
			sik.add(node);
            preOrder(node.left, sik);           
            preOrder(node.right, sik);
        }
    }
	
	/* Function for postOrder traversal */
	public List<Node<Key,Value>> postOrder() {
        List<Node<Key,Value>> sipo = new ArrayList<>();
        postOrder(root, sipo);
        return nod;
    }

    private void postOrder(Node<Key,Value> node, List<Node<Key,Value>> sipo) {
        if(node != null) {			
            postOrder(node.left, sipo);           
            postOrder(node.right, sipo);
			nek.add(node);
        }
    }
	
	/* Function for successor */
	public Key successor(Key key) {
		Key sucrKey = key;	
		if (root == null)
			return key;
		
        Node<Key,Value> sisu = get(root, key); 			
		if(sisu != null){
			if (sisu.right != null) {
				sisu = minimum(sisu.right);
				sucrKey = sisu.key;
			}
			
			Node<Key,Value> par = sisu.parent;
			while (par != null && sisu == par.right) {
				sisu = par;
				par = par.parent;
			}
	...
	public Key predecessor(Key key) {
        ...
        Node<Key,Value> mnt = get(root, key); 			
		if(mnt != null){
			if (mnt.left != null) {
				mnt = maximum(nods.left);
				predrKey = mnt.key;
			}
			
			Node<Key,Value> par = mnt.parent;
			while (par != null && mnt == par.left) {
				mnt = par;
				par = par.parent;
			}
    ...
    private void toStringRecursive(Node<Key, Value> sid, StringBuilder strBuilder) {
        if(sid != null) {
            strBuilder.append(sid.key);
            if(sid.left != null || sid.right !=null) {
                strBuilder.append("(");
                toStringRecursive(sid.left, strBuilder);
                strBuilder.append(",");
                toStringRecursive(sid.right, strBuilder);
                strBuilder.append(")");
            }
        }
        else{
    ...
    public String toString() {
		...
		toStringRecursive(root, strBuilder);
